...
class Critic(nn.Module):
    # 输入 (batch_size * seq_len * hidden_size) * (hidden_size * 1) = batch_size * action_len * 1
    ...
    def forward(self, input_ids, attention_mask, num_actions):
        ...
        values = value_model_output.squeeze(-1)[:, -num_actions:]  # 只把模型生成的那部分价值取出来
        return values
...
def compute_value_loss(values, old_values, returns, action_mask=None, clip_eps: float = None):
    ...
    if clip_eps is not None:
        ...
        loss = (values - returns) ** 2  # 最简单情况应该是这个
...
def get_advantages_and_returns(
        ...
        gamma: float,
        lambd: float):
    # 广义优势函数gae,通过values和rewards估计advantage和returns
...
def generate_samples(prompts, model, max_length, max_new_tokens, n_samples_per_prompt, micro_rollout_batch_size):
    ...
    # 每个prompt复制n_samples_per_prompt=2份f
    for i in range(0, len(all_prompts), micro_rollout_batch_size):  # micro_rollout_batch_size 2
        ...
        # seqs 2 * 306,每一条samples有两条生成的样本，prompt + answer
...
def generate_experiences(samples_list):
    ...
    for samples in samples_list:
        ...
        with torch.no_grad():
            ...
            # action的kl距离，最后一个位置为reward模型给出的奖励
            # 计算优势和回报
